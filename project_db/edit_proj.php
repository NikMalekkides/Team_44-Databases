...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' ) {
    ...
    $evaluator_grade = $row["evaluator_grade"];
    ...
    $managing_date = $row["managing_date"];

    $sql = "SELECT field_name FROM field WHERE project_id=$id";
    $result = $connection->query($sql);
    ...
    do {
        if ( empty($title) || empty($summary) || empty($start_date) || empty($end_date) || empty($evaluator_id)
            || empty($evaluator_grade) || empty($evaluation_date) || empty($supervisor_id) || empty($controller_id)
            || empty($sponsor_id) || empty($sponsor_amount) || empty($managing_org_id) || empty($managing_date) || empty($field_name) )  {
        ...
        $sql = "UPDATE project " .
               ...
               "evaluator_id = '$evaluator_id', evaluator_grade = '$evaluator_grade', evaluation_date = '$evaluation_date', " .
               "supervisor_id = '$supervisor_id', controller_id = '$controller_id', sponsor_id = '$sponsor_id', " .
               "sponsor_amount = '$sponsor_amount', managing_org_id = '$managing_org_id', managing_date = '$managing_date' " .
               "WHERE id = $id";
        ...
        $successMessage = "Project updated correctly";
    ...
    <div class="container my-5">
        ...
        <form method="post" id="curr">
            ...
            <fieldset class="border p-2">
                ...
                <br>
                <div>
                <input type="checkbox" id="field_name[]" name="field_name[]" value='ΦΥΣΙΚΕΣ ΕΠΙΣΤΗΜΕΣ' <?php if(isset($field_name["ΦΥΣΙΚΕΣ ΕΠΙΣΤΗΜΕΣ"])) echo "checked" ?>>
                <label for='ΦΥΣΙΚΕΣ ΕΠΙΣΤΗΜΕΣ'>Φυσικές Επιστήμες</label>
                </div>
                <div>
                <input type="checkbox" id="field_name[]" name="field_name[]" value='ΤΕΧΝΟΛΟΓΙΑ' <?php if(isset($field_name["ΤΕΧΝΟΛΟΓΙΑ"])) echo "checked" ?>>
                <label for='ΤΕΧΝΟΛΟΓΙΑ'>Τεχνολογία</label>
                </div>
                <div>
                <input type="checkbox" id="field_name[]" name="field_name[]" value='ΙΑΤΡΙΚΗ' <?php if(isset($field_name["ΙΑΤΡΙΚΗ"])) echo "checked" ?>>
                <label for='ΙΑΤΡΙΚΗ'>Ιατρική</label>
                </div>
                <div>
                <input type="checkbox" id="field_name[]" name="field_name[]" value='ΓΕΩΠΟΝΙΚΗ' <?php if(isset($field_name["ΓΕΩΠΟΝΙΚΗ"])) echo "checked" ?>>
                <label for='ΓΕΩΠΟΝΙΚΗ'>Γεωπονική</label>
                </div>
                <div>
                <input type="checkbox" id="field_name[]" name="field_name[]" value='ΠΛΗΡΟΦΟΡΙΚΗ' <?php if(isset($field_name["ΠΛΗΡΟΦΟΡΙΚΗ"])) echo "checked" ?>>
                <label for='ΠΛΗΡΟΦΟΡΙΚΗ'>Πληροφορική</label>
                </div>
                <div>
                <input type="checkbox" id="field_name[]" name="field_name[]" value='ΚΟΙΝΩΝΙΚΕΣ ΕΠΙΣΤΗΜΕΣ' <?php if(isset($field_name["ΚΟΙΝΩΝΙΚΕΣ ΕΠΙΣΤΗΜΕΣ"])) echo "checked" ?>>
                <label for='ΚΟΙΝΩΝΙΚΕΣ ΕΠΙΣΤΗΜΕΣ'>Κοινωνικές Επιστήμες</label>
                </div>
                <div>
                <input type="checkbox" id="field_name[]" name="field_name[]" value='ΤΕΧΝΕΣ' <?php if(isset($field_name["ΤΕΧΝΕΣ"])) echo "checked" ?>>
                <label for='ΤΕΧΝΕΣ'>Τέχνες</label>
                </div>
                <div>
                <input type="checkbox" id="field_name[]" name="field_name[]" value='ΟΙΚΟΝΟΜΙΚΑ' <?php if(isset($field_name["ΟΙΚΟΝΟΜΙΚΑ"])) echo "checked" ?>>
